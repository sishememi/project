...
<script type="text/javascript">
	...
	function searchlist(wsection) {
		if(wsection=="전체"){
			location.href='tourForm.do?bsection=2'
		}else {
...
<form action ="restaurantForm.do?bsection=2" method="post" name="sr">
<table class="board" >
<tr><td colspan="6"><h3>Restaurant</h3></td></tr>
   <c:if test="${boardcnt == 0}">
      ...
      <tr> 
      	 <td>국가 선택</td>
      	 <td>
      	 <select name="column" class="nation2" onchange="searchlist(this.value)">
      ...
      <div class="table101-head">
      <tr class="h">
         <th width="8%" class="hh">번호</th>
         <th width="5%" class="hh">&nbsp;</th>
         <th width="46%" class="hh">제목</th>
         <th width="15%" class="hh">작성자</th>
         <th width="20%" class="hh">작성일</th>
         <th width="11%" class="hh">조회</th>
      </tr>
      </div>
       <c:forEach var="b" items="${list}">
        <tr class="r"><td>${boardnum}</td> <!-- boardnum : 보여주기 식 번호 -->
        <c:set var="boardnum" value="${boardnum -1 }" />
      	<td style="size:10px;">${b.wsection}</td>
        <td><a href="info.do?bsection=2&bnum=${b.bnum}" class="sub">${b.subject}</a>
         </td><td>${b.id}</td>
         <td>
...
<c:if test="${pageNum < maxpage}">
	<a href="javascript:listdo(${pageNum + 1})">다음</a> 
      ...
      <tr>
         <td colspan="6" style="text-align: right"><a href="reswrite.do?bsection=2" class="bb">글쓰기</a></td>
      </tr>
</table>
</form>
<form action ="restaurantForm.do?bsection=2" method="post" name="sh">
...
<input type="submit" class="searchform2" value="검색">
</tr> 
</table>
</form>
</body>
